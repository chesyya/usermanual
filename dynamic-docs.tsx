...
// API配置
const API_BASE_URL = 'http://localhost:5556/usermanual/plugins/structure';
...
useEffect(() => {
  // 从 URL 中解析 docId
  const docId = decodeURIComponent(location.pathname.replace(/^\/dynamic\//, ''));
  ...
  const fetchStructure = async () => {
    try {
      ...
      console.dir(data);
      if (data.success) {
        setStructure(data.content);
        // 自动加载第一个文档
        const firstDoc = findFirstDoc(data.content);
        if (firstDoc) {
  ...
  const renderSidebarItem = (item: DocItem, level = 0) => {
    console.dir(`Rendering sidebar item:`);
